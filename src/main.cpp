...
int main() {
    ...
    hero.inventory.addWeapon(new Weapon("Excalibur", 25));
    hero.inventory.addArmor(new Armor("Dragon Shield", 15));

    dragon.inventory.addWeapon(new Weapon("Dragon Claws", 30));
    dragon.inventory.addArmor(new Armor("Dragon Scales", 20));
    ...
    dragon.attack(hero);
    cout << dragon.name<< " counterattacks:\n" << hero << dragon << "----------------\n";
    ...
    hero.inventory.addWeapon(generateRandomWeapon());
    hero.inventory.addArmor(generateRandomArmor());
    
    Weapon* fireBreath = new Weapon("Fire Breath", 40);
    Armor* solziDeDragon = new Armor("Solzi de Dragon", 25);
    
    dragon.inventory.addWeapon(fireBreath);
    dragon.inventory.addArmor(solziDeDragon);

    while (hero.getHealth() > 0 && dragon.getHealth() > 0) {
        hero.attack(dragon);
        cout << hero.name << " attacks Dragon:\n" << hero << dragon ;

        if (dragon.getHealth() <= 0) {
            std::cout << dragon.name <<" has been defeated! "<< hero.name <<" wins!\n";
            break;
        }
        ...
        dragon.attack(hero);
        cout << dragon.name <<" attacks Hero:\n" << hero << dragon;

        if (hero.getHealth() <= 0) {
            cout << hero.name << " has been defeated! "<< dragon.name << " wins!\n";
            break;
        }
